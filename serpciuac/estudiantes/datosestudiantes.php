...
session_start();   
  ...
  */
  ...
  $idUsuario = traerIDUsuario($varsesion,$varusuario);
  $idIdioma= traerIDIdioma($idUsuario);
  $idiomanombre =traerIdioma($idIdioma);
  $gradonombre= traergrado($idUsuario);
  $horarionombre= traerhorario($idUsuario);
  $usuario = $varsesion;
...
function traerIDUsuario($usuario,$clave){
  ...
  function traergrado($usuario){
    $conexion= mysqli_connect("localhost","root","","idiomas");
    
    $sql = "SELECT nombre_grado from grados WHERE Id_Grado = (SELECT Id_Grado from matriculados WHERE id_usuario = '$usuario')";
      $result = mysqli_query($conexion,$sql);
      return mysqli_fetch_row($result)[0];
  }

  function traerhorario($usuario){
    
    $conexion= mysqli_connect("localhost","root","","idiomas");
    $conexion -> set_charset("utf8");
    $sql = "SELECT Nombre_Horario from horario WHERE Id_Horario = (SELECT Id_Horario from matriculados WHERE id_usuario = '$usuario')";
      $result = mysqli_query($conexion,$sql);
      return mysqli_fetch_row($result)[0];
  }

  function traerIDIdioma($id_usuario){
    $conexion= mysqli_connect("localhost","root","","idiomas");
    $sql = "SELECT Id_Idioma FROM matriculados WHERE id_usuario = '$id_usuario'";
      $result = mysqli_query($conexion,$sql);
      return mysqli_fetch_row($result)[0];
  }

  function traerIdioma($id_idioma){
    $conexion= mysqli_connect("localhost","root","","idiomas");
    $sql = "SELECT Nombre_Idioma FROM idioma WHERE Id_idioma = '$id_idioma'";
      $result = mysqli_query($conexion,$sql);
      return mysqli_fetch_row($result)[0];
  }
